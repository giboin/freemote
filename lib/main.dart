...
class _MyAppState extends State<MyApp> {
  ...
  ThemeMode _themeMode = SchedulerBinding.instance.window.platformBrightness==Brightness.light?ThemeMode.light:ThemeMode.dark;

  Map buttonIsActive={
    "vol_inc":true, "red":true, "up":true, "blue":true, "prgm_inc":true,
    "vol":true, "left":true, "OK":true, "right":true, "prog":true,
    "vol_dec":true, "green":true, "down":true, "yellow":true, "prgm_dec":true,
    "1":true, "2":true, "3":true, "4":true, "5":true,
    "6":true, "7":true, "8":true, "9":true, "0":true,
    "bwd":true, "play":true, "fwd":true, "random":true, "rec":true,
    "mute":true, "home":true, "power":true,
    "lock":false
  };
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
        ...
        darkTheme: ThemeData(
            ...
            textTheme: TextTheme.lerp(Typography.blackCupertino, Typography.whiteCupertino, 0.7),
            iconTheme: IconThemeData(color: Colors.grey[500])
        ),
        themeMode: _themeMode,
        home:Scaffold(
            appBar: AppBar(
                title:const Text("freebox"),
                actions:[
                  IconButton(
                    icon:Icon(_themeMode==ThemeMode.dark?Icons.light_mode:Icons.dark_mode_outlined),
                    onPressed: (){
                      setState(() {
                        _themeMode=_themeMode==ThemeMode.dark?ThemeMode.light:ThemeMode.dark;
                        _saveThemeMode(_themeMode==ThemeMode.dark?"dark":"light");
                      });
                    },
                  ),
                  IconButton(
            ...
            body:OrientationBuilder(
                builder: (context, orientation) {
                  if (orientation == Orientation.portrait) {
                    return Column(children:showPad?[
                      remote(iconSize),
                      pad(),
                    ]:[remote(iconSize)]);
                  } else {
                    return Row(children:showPad?[
          ...
          children: [
            Row(
              ...
              children: [
                IconGesture(
                  ...
                  color:_themeMode==ThemeMode.light?Colors.black:Colors.grey[300]!,
                  active: buttonIsActive["vol_inc"]!,
                  onPressed: (){sendOrder("vol_inc");},
            ...
            Row(
              ...
              children: [
                ...
                IconGesture(
                  icon: Text("prog", style:TextStyle(fontSize: iconSize/2.5)),
                  color:_themeMode==ThemeMode.light?Colors.black:Colors.grey[300]!,
            ...
            Row(
              ...
              children: [
                IconGesture(
                  ...
                  onPressed: (){sendOrder("bwd");},
                  onLongPress: (){sendOrder("prev");},
                  iconSize: iconSize,
                ),
                IconGesture(
                ...
                IconGesture(
                  ...
                  onPressed: (){sendOrder("fwd");},
                  onLongPress: (){sendOrder("next");},
                  iconSize: iconSize,
                ),
                IconGesture(
            ...
            Row(
              ...
              children: [
                ...
                OutlinedButton(
                    style: !buttonIsActive["home"]!?const ButtonStyle(splashFactory: NoSplash.splashFactory):const ButtonStyle(),
                    onPressed: (){
                    ...
                    child: Container(
                      ...
                      child: Text("free", style:TextStyle(fontSize: 30, color: (buttonIsActive["home"]!)?Colors.redAccent:Colors.grey, fontStyle: FontStyle.italic)),
                    )
                ),
                IconGesture(
    ...
    return Expanded(
        child:GestureDetector(
            child: Container(
                ...
                decoration: BoxDecoration(borderRadius: const BorderRadius.all(Radius.circular(15)),color:_themeMode==ThemeMode.dark?Colors.grey[800]:Colors.blue),
                child: InfoButton()
            ),
            onTap: (){
              sendOrder("ok");
                    ...
                    return AlertDialog(
                        ...
                        content: SizedBox(
                            ...
                            child:SingleChildScrollView(child:Column(
                              children: [
                                const Text("Certains boutons de la télécommande peuvent être remplacés par des gestes sur le pad de control, pour une navigation plus rapide dans les applications de la freebox:"),
                                ListView.separated(
                                  physics: const NeverScrollableScrollPhysics(),
                                  shrinkWrap: true,
                                  separatorBuilder: (BuildContext context, int index) => const Divider(height: 3, color: Colors.grey),
                                  itemCount: 7,
                                  itemBuilder: (BuildContext context, int index) =>[
                                    ...
                                    const ListTile(
                                      leading: Icon(Icons.subdirectory_arrow_left_rounded, color:Colors.red),
                                ...
                                const Text("Le bouton pour accéder à ce menu peut être désactivé dans les paramètres"),
                                Row(children:[Expanded(child:Container()),ElevatedButton(onPressed: (){Navigator.pop(ctx);}, child: const Text("Ok"))])
                              ],
                            ),
                            )
                        )
  ...
  void longPress (String key, int delay) {
    Future.delayed(Duration(milliseconds: delay),() async {
      ...
      if(mustContinue){
        http.get(Uri.parse('http://hd1.freebox.fr/pub/remote_control?key=$key&code=$code'));
        longPress(key, delay);
      }
    });
  ...
  Future<void> sendOrder(String order) async {
    ...
    if(code==""){
      await showDialog(context: context, builder: (BuildContext context){
        return const AlertDialog(title:Text("pas de code"), content: Text("Veuillez renseigner le code de la télécommande dans les parametres"),);
      });
      return;
    }
    if(order!="random") {
      http.get(Uri.parse('http://hd1.freebox.fr/pub/remote_control?key=$order&code=$code'));
    }else{
      var random=Random();
      int rand=random.nextInt(49)+1;
      if(rand<10){
        http.get(Uri.parse('http://hd1.freebox.fr/pub/remote_control?key=$rand&code=$code')); //7177533
      }
      else{
        http.get(Uri.parse('http://hd1.freebox.fr/pub/remote_control?key=${rand~/10}&code=$code'));
        http.get(Uri.parse('http://hd1.freebox.fr/pub/remote_control?key=${rand-(rand~/10)*10}&code=$code'));
      }
    }
  }
